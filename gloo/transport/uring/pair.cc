...
#include "gloo/transport/uring/pair.h"
...
#include "gloo/transport/uring/buffer.h"
#include "gloo/transport/uring/context.h"
#include "gloo/transport/uring/unbound_buffer.h"
...
Pair::Pair(Context *context, Device *device, int rank,
    ...
    : context_(context), device_(device), rank_(rank), state_(INITIALIZING),
      sync_(false), timeout_(timeout), busyPoll_(false), fd_(FD_INVALID),
      sendBufferSize_(0), is_client_(false), ex_(nullptr) {
  listen();
}
...
const Address &Pair::address() const { return self_; }
...
ssize_t Pair::prepareWrite(Op &op, const NonOwningPtr<UnboundBuffer> &buf,
                           struct iovec *iov, int &ioc) {
  ssize_t len = 0;
  ioc = 0;
...
//
ssize_t Pair::prepareRead(Op &op, NonOwningPtr<UnboundBuffer> &buf,
                          struct iovec &iov) {
...
void Pair::registerBuffer(Buffer *buf) {
  std::lock_guard<std::mutex> lock(m_);
  GLOO_ENFORCE(buffers_.find(buf->slot_) == buffers_.end(),
               "duplicate buffer for slot ", buf->slot_);
...
void Pair::verifyConnected() {
  ...
  GLOO_ENFORCE_GE(state_, CONNECTED, "Pair is not connected (", self_.str(),
                  " <--> ", peer_.str(), ")");
...
void Pair::recv() {
  ...
  if (!rv) {
    GLOO_ENFORCE(ex_ != nullptr,
                 "read() returned false in sync mode; ex_ must be set");
    std::rethrow_exception(ex_);
  }
}
...
void Pair::send(transport::UnboundBuffer *tbuf, uint64_t slot, size_t offset,
                size_t nbytes) {
  auto buf = static_cast<uring::UnboundBuffer *>(tbuf)->getWeakNonOwningPtr();
...
void Pair::recv(transport::UnboundBuffer *tbuf, uint64_t slot, size_t offset,
                size_t nbytes) {
  auto buf = static_cast<uring::UnboundBuffer *>(tbuf)->getWeakNonOwningPtr();
...
bool Pair::tryRecv(transport::UnboundBuffer *tbuf, uint64_t slot, size_t offset,
                   size_t nbytes) {
  auto buf = static_cast<uring::UnboundBuffer *>(tbuf)->getWeakNonOwningPtr();
...
void Pair::sendUnboundBuffer(WeakNonOwningPtr<UnboundBuffer> buf, uint64_t slot,
