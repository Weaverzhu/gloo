...
UnboundBuffer::UnboundBuffer(const std::shared_ptr<Context> &context, void *ptr,
                             size_t size)
    : ::gloo::transport::UnboundBuffer(ptr, size), context_(context),
      recvCompletions_(0), recvRank_(-1), sendCompletions_(0), sendRank_(-1),
  ...
  if (recvCompletions_ == 0) {
    ...
    if (!done) {
      ...
      throw ::gloo::IoException(
          GLOO_ERROR_MSG("Timed out waiting ", timeout.count(),
