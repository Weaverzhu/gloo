...
std::unique_ptr<transport::Pair> &Context::createPair(int rank) {
  pairs_[rank] = std::unique_ptr<transport::Pair>(
      new uring::Pair(this, device_.get(), rank, getTimeout()));
  return pairs_[rank];
}

std::unique_ptr<transport::UnboundBuffer>
Context::createUnboundBuffer(void *ptr, size_t size) {
  auto buf = new uring::UnboundBuffer(shared_from_this(), ptr, size);
...
void Context::signalException(const std::string &msg) {
  ...
  for (auto &pair : pairs_) {
    if (pair) {
      reinterpret_cast<uring::Pair *>(pair.get())->signalExceptionExternal(msg);
    }
  }
}
