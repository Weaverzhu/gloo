...
#include "gloo/transport/uring/address.h"
#include "gloo/transport/uring/device.h"
...
public:
  ...
  virtual void send(transport::UnboundBuffer *tbuf, uint64_t tag, size_t offset,
                    size_t nbytes) override;

  // Receive into the specified buffer from the remote side of pair.
  virtual void recv(transport::UnboundBuffer *tbuf, uint64_t tag, size_t offset,
                    size_t nbytes) override;
  ...
  bool tryRecv(transport::UnboundBuffer *tbuf, uint64_t tag, size_t offset,
               size_t nbytes);
...
protected:
  ...
  void sendUnboundBuffer(WeakNonOwningPtr<UnboundBuffer> buf, uint64_t slot,
...
protected:
  ...
  ssize_t prepareWrite(Op &op, const NonOwningPtr<UnboundBuffer> &buf,
  ...
  ssize_t prepareRead(Op &op, NonOwningPtr<UnboundBuffer> &buf,
                      struct iovec &iov);
  ...
  void waitUntil(pred_t pred, std::unique_lock<std::mutex> &lock,
    ...
    if (useTimeout && timeoutSet) {
      ...
      auto relTime = std::min(
          timeout_ * 5, std::chrono::duration_cast<std::chrono::milliseconds>(
                            kLargeTimeDuration));
